(async function() {
    ...
    const displayValueA = createSetting("Display Value on Badge Page");
    
    const settingE = "displayValueA";
    addButton(displayValueA, settingE, coalesce(await getSetting(settingE), false), function(value) {
        setSetting(settingE, value);
    });

    document.body.appendChild(displayValueA);

    const displayValueB = createSetting("Display Value on Inventory Page");
    
    const settingF = "displayValueB";
    addButton(displayValueB, settingF, coalesce(await getSetting(settingF), false), function(value) {
        setSetting(settingF, value);
    });

    document.body.appendChild(displayValueB);
